...
module.exports = async function globalSetup() {
  ...
  try {
    ...
    await client.query(
      `TRUNCATE TABLE portal_setting_changes, notification_policy_changes, notifications, consumer_receipts, outbox_events, audit_log,
              idempotency_records, submissions, work_item_fields, work_items,
              sessions RESTART IDENTITY CASCADE`,
    );
    await client.query('TRUNCATE service_intake_runs,service_intake_authorizations,service_intake_actors,focus_slots,focus_configurations,scoring_weights,scoring_models,vehicle_stock_rows,manager_discount_rows,vehicle_identity,report_detail_publications,report_detail_previews,report_detail_access,metric_deviation_tasks,report_fact_current,report_fact_snapshots,report_fact_publications,report_fact_previews,report_source_scans,report_fact_access,report_review_revisions,report_staging_access,report_staging_files,report_staging_batches');
    ...
    await client.query('TRUNCATE daily_log_links,daily_submission_markers,daily_log_records,daily_log_policies CASCADE');
