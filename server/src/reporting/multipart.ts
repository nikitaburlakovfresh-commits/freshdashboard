...
import { UploadFile, MAX_FILE_BYTES, MAX_BATCH_FILES } from './storage';
...
export async function readUpload(req:Request):Promise<{metadata:unknown;files:UploadFile[]}> {
  ...
  return new Promise((resolve,reject)=>{
    ...
    try {parser=Busboy({headers:req.headers,limits:{fileSize:MAX_FILE_BYTES,files:MAX_BATCH_FILES,fields:1,fieldSize:4096,parts:MAX_BATCH_FILES+2,headerPairs:30}});}
    ...
    for(const event of ['filesLimit','fieldsLimit','partsLimit']) parser.on(event,()=>fail(`Допустимы 1 поле метаданных и до ${MAX_BATCH_FILES} файлов.`));
