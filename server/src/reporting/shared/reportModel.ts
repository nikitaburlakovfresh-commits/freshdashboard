...
export const REPORT_SPECS: Record<ReportKind, ReportSpec> = {
  ...
  /** Ручная форма плана выручки: филиал и плановая выручка.
   *
   * Строка итога обязательна. План — аддитивный показатель, а публикация
   * аддитивного показателя без объявленного итога не проходит сверку: портал
   * не станет считать итог сам и выдавать свою сумму за официальную. Для
   * ручной формы это единственный контроль полноты — загружающий объявляет
   * итог сети, портал сверяет его с суммой строк и расходится с ним громко. */
  revenuePlan: {
    headerRow: 1, hasTotalRow: true,
