...
reportFactsRouter.post('/:id/scan',requireOrigin,requireCsrf,wrap(async(req,res)=>{res.json(await scanBatch(req.authUser!,req.params.id,req.body,req.ctx.requestId));}));
reportFactsRouter.post('/:id/preview',requireOrigin,requireCsrf,wrap(async(req,res)=>{res.json(await previewPublication(req.authUser!,req.params.id,req.body));}));
reportFactsRouter.post('/:id/publish',requireOrigin,requireCsrf,wrap(async(req,res)=>{res.json(await commitPublication(req.authUser!,req.params.id,req.body,req.get('Idempotency-Key'),req.ctx.requestId));}));
