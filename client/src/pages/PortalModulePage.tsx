...
export default function PortalModulePage(){
  ...
  return <div className="portal-dashboard">
    ...
    <section className="portal-panel"><h2>Состояние подключения</h2><p>Это навигационный каркас, а не готовый расчётный модуль. Реальные значения, графики, импорт и сохранение данных пока отсутствуют.</p>{metric&&<p className="portal-muted">Код: {metric.code} · предусмотренный ТЗ источник: {metric.source}. Фактической загрузки ещё не было.</p>}<p>Ноль не подставляется вместо отсутствующих данных. Завершение задачи не означает автоматическое достижение KPI или финансовой цели.</p></section>
