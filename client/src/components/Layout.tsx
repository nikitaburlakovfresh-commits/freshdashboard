  ...
  const nav = <>
    ...
    <nav className="shell-nav" aria-label="Основная навигация">
      {groups.map(group => <div className="shell-nav-group" key={group.label}>
        ...
        {group.links.filter(link=>!['/prepared-reports','/saved-network'].includes(link.path) || me?.grants.some(g=>g.role==='SUPER_ADMIN')).map(link => <NavLink key={link.path} to={link.path} end={link.path === '/'} onClick={close}
