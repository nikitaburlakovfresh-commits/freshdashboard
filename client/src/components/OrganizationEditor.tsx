...
const labels={ORG_UNIT_CREATE:'Создание единицы',ORG_UNIT_RENAME:'Переименование',ORG_UNIT_MOVE_TO_CLUSTER:'Перенос к другому родителю',ORG_UNIT_ACTIVATE:'Активация филиала'};
const operationPermissions={ORG_UNIT_CREATE:'org_unit.create',ORG_UNIT_RENAME:'org_unit.rename',ORG_UNIT_MOVE_TO_CLUSTER:'org_unit.move',ORG_UNIT_ACTIVATE:'org_unit.activate'};
...
export default function OrganizationEditor({units,onApplied}:{units:DirectoryUnit[];onApplied:()=>void}) {
  ...
  const permission=operationPermissions[change.operation];
  const activation=change.operation==='ORG_UNIT_ACTIVATE';
  ...
  function newOperation(operation:OrgChange['operation']) {
    setChange(operation==='ORG_UNIT_CREATE'?empty():{operation,target_id:'',effective_from:new Date().toISOString().slice(0,10),reason:'',
      ...(operation==='ORG_UNIT_RENAME'?{display_name:''}:operation==='ORG_UNIT_ACTIVATE'?{}:{parent_id:null})});
    setDirty(true);
  }
  const eligible=units.filter(u=>!u.is_demo && !u.demo_locked && u.lifecycle_state==='PRE_LAUNCH' && (!activation || u.kind==='ORG_UNIT'));
  return <section className="portal-panel org-editor" aria-label="Редактор оргструктуры">
    ...
    <p className="portal-muted">Создание, переименование и перенос единиц до запуска. Активация реального филиала требует отдельного права и подтверждения. A/B защищены; роли, показатели и импорт автоматически не меняются.</p>
    <div className="org-editor-grid">
      ...
      <div className="org-editor-content">
        ...
        <label className="org-operation-field">Операция<select aria-label="Операция изменения" data-testid="org-operation" value={change.operation} disabled={!!p || busy} onChange={e=>newOperation(e.target.value as OrgChange['operation'])}>
          {Object.entries(labels).map(([key,value])=><option key={key} value={key} disabled={!permissions.has(operationPermissions[key as OrgChange['operation']])}>{value}</option>)}</select></label>
        <fieldset disabled={busy || !writable} className="org-editor-fields">
          ...
          </>:<label>{activation?'Филиал до запуска':'Единица до запуска'}<select aria-label="Единица до запуска" data-testid="org-target" value={change.target_id} disabled={!!p} onChange={e=>update({target_id:e.target.value})}>
            <option value="">Выберите единицу</option>{change.target_id && !eligible.some(u=>u.id===change.target_id) && <option value={change.target_id}>{units.find(u=>u.id===change.target_id)?.display_name ?? change.target_id}</option>}{eligible.map(u=><option key={u.id} value={u.id}>{u.display_name} · {u.code}</option>)}</select></label>}
          {['ORG_UNIT_CREATE','ORG_UNIT_RENAME'].includes(change.operation) && <label>Название<input data-testid="org-name" value={change.display_name ?? ''} maxLength={200} onChange={e=>update({display_name:e.target.value})}/></label>}
          {!activation && change.operation!=='ORG_UNIT_RENAME' && !(change.operation==='ORG_UNIT_CREATE' && change.kind==='NETWORK') &&
          ...
          <label>Дата вступления в силу (UTC)<input data-testid="org-effective-date" type="date" value={change.effective_from} min={new Date().toISOString().slice(0,10)} max={activation?new Date().toISOString().slice(0,10):'9999-12-31'} onChange={e=>update({effective_from:e.target.value})}/></label>
        ...
        {!activation && <p className="org-small portal-muted">Новые единицы: только «До запуска». Тип и бизнес-модель можно оставить неподтверждёнными. Для смены имени/родителя дата должна быть позже начала последнего интервала. Будущие единицы появятся в срезе на дату вступления в силу.</p>}
        {activation && <p className="org-small portal-muted">Активация действует после применения, только текущей датой UTC. Нужны филиал, созданный редактором, действующее название и подтверждённая цепочка родителей. Это не публикация KPI и не выдача доступа. Отмена, приостановка и закрытие пока не поддерживаются.</p>}
        ...
        {p?.preview_summary && <section className="org-preview-summary" aria-label="Результат проверки">
          ...
          {p.change.operation==='ORG_UNIT_ACTIVATE' && <p><strong>До запуска → Активен.</strong> Записей истории запуска: +{p.preview_summary.affected.lifecycle_history}. Дата среза ранее запуска сохранит прежний статус.</p>}
    ...
    <dialog ref={dialog} className="org-apply-dialog" aria-labelledby="org-apply-title">
      ...
      <p>{p && labels[p.change.operation]} · {p?.change.display_name || units.find(u=>u.id===p?.target_id)?.display_name || p?.target_id} · с {p?.change.effective_from}</p>
      ...
      {p?.change.operation==='ORG_UNIT_ACTIVATE' && <p>Филиал станет активным. Создание новых задач будет разрешено только пользователям с отдельным назначением. KPI, рассылки и назначения автоматически не создаются.</p>}
